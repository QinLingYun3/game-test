...
- [ ] 游戏开始后可以从道具栏拖放混乱到对手头像，成功触发效果
- [ ] 双击混乱道具自动对分数最高的其他玩家生效
- [ ] 被击中者所有**顶层** tile 图标以 4 图标轮换（本身停 0.5s，其余各 0.2s），持续 6s
- [ ] 混乱期间 tile 仍可正常点击、配对、消除（逻辑基于真实 type）
- [ ] 6s 到期后所有 tile 图标立即恢复为本身样式
- [ ] 生效期间被击中玩家昵称下方显示 😵‍💫 图标
- [ ] 同一目标连续使用混乱时，后续进入队列，当前结束后自动生效
- [ ] 玩家离开房间或回到大厅后，混乱效果及标记完全清除
- [ ] 测试模式下可本地触发混乱效果看动画
